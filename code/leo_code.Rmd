...
```{r}
# read in the data
df_orig_raw <- st_read(file.path(indir, "right_whale_SMA_all_po.shp")) %>% 
  clean_names()
...
```{r}
df_final <- stur_all %>%  
  mutate(dataset = "Atlantic Sturgeon critical Habitat",
        country = "USA",
        ocean = "Atlantic",
        region = leadoffice,
        agency = "NOAA",
        ...
        species_list = NA,
        source = "https://www.fisheries.noaa.gov/resource/map/atlantic-sturgeon-critical-habitat-map-and-gis-data") %>% 
  select(dataset, country, ocean, region, agency, zone_id, zone_name, type, species, species_list, source) %>% 
 st_transform(wgs84)
```

```{r}
# Create output RDS file
saveRDS(df_final, file = file.path(outdir, "USA_atlantic_sturgeon_critical_habitat.Rds"))
```

```{r}
ggplot() +
  geom_sf(data = world) +
  geom_sf(data = stur_all, fill = NA) +
