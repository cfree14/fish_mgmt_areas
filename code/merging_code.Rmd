...
perudir <- file.path(basedir, "peru", "processed")
...
peru_updated <- st_read(file.path(out_final, "Peru_fishing_zones.gpkg"))

canada_all <- list.files(file.path(outdir),
                    pattern = "*.Rds",
                    full.names = TRUE)

canada_all_tbl <- sapply(canada_all, readRDS, simplify = FALSE) 

canada_all_tbl_2 <- canada_all_tbl %>% 
  bind_rows() %>% 
...
df_final <- canada_all_tbl_2  %>%   
  mutate(pk_system_history = NA,
         ...
         source_date = created,
         usage = NA,
         lineage = "Downloaded and imported from the DFO and PSMFC on 2022-07-28",
         comment = NA,
  ...
  select(-dataset, -country, -ocean, -region, -agency, -zone_id, -zone_name, -species, -type, -citation, -created) 
```

```{r}
# export shapefile
st_write(df_final, file.path(out_final, "Canada_mgmt_areas.gpkg"))
```
