...
chiledir <- file.path(basedir, "chile", "processed")
...
chile <- list.files(file.path(chiledir),
                    pattern = "*.Rds",
                    full.names = TRUE)

chile_tbl <- sapply(chile, readRDS, simplify = FALSE) 

chile_tbl_2 <- chile_tbl %>% 
  bind_rows() %>% 
...
df_final <- chile_tbl_2 %>%  
  mutate(pk_system_history = NA,
         ...
         source_date = created,
         usage = NA,
         lineage = "Downloaded and imported from the Ministry of National Goods of Chile on 2022-07-28",
         comment = NA,
  ...
  select(-dataset, -country, -ocean, -region, -agency, -zone_id, -zone_name, -species, -type, -species_list, -citation, -dataset_esp) 
```

```{r}
# export shapefile
st_write(df_final_col, file.path(out_final, "Colombia_shrimp_fisheries_zoning2.gpkg"))
```
