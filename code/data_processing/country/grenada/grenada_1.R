...
data_orig <- st_read(file.path(basedir, "raw", "MR_EEZ", "eez_v11.shp")) %>%
  filter(grepl("Grenadian", GEONAME)) %>%
...
data <- data_orig %>%
  ...
  mutate(
    ...
    System_name_english = "Grenada", # required
    System_code = "grd", # required
    ...
    System_lineage = "Filtered eez_v11.shp to so that GEONAME contained \"Grenadian\"", # required
    System_type = "Grenada EEZ", # required
    System_category = "Management Area", # required
    Area_systematic_name_english = "Grenada EEZ", # required
...
saveRDS(data, file = file.path(basedir, "processed", "country", "grenada", "grenada_1.Rds"))
