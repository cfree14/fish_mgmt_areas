...
data_orig <- st_read(file.path(basedir, "raw", "MR_EEZ", "eez_v11.shp")) %>%
  filter(grepl("Madagascar", GEONAME)) %>%
...
data <- data_orig %>%
  ...
  mutate(
    ...
    System_name_english = "Madagascar", # required
    System_code = "mdg", # required
    ...
    System_lineage = "Filtered eez_v11.shp to so that GEONAME contained \"Madagascar\"", # required
    System_type = "Madagascar EEZ", # required
    System_category = "Management Area", # required
    Area_systematic_name_english = "Madagascar EEZ", # required
...
saveRDS(data, file = file.path(basedir, "processed", "country", "madagascar", "madagascar_1.Rds"))
