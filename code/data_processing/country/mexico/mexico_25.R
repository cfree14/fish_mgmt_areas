...
# Read data
data_orig <- st_read(file.path(basedir, "raw", "country", "mexico", "barrilete.shp")) %>%
...
data = data %>% # required
  ...
  mutate(
    ...
    Owner_country = "mex",
    System_name_english = "National Fishing Card for Barrilete", # required
    ...
    System_shape_file = "barrilete.shp",
    ...
    Area_systematic_name_english = "Capture Area for Barrilete", # required
...
saveRDS(data, file = file.path(basedir, "processed", "country", "mexico", "mexico_25.Rds"))
