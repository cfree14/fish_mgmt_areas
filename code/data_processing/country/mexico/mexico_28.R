...
# Read data
data_orig <- st_read(file.path(basedir, "raw", "country", "mexico", "pez_vela.shp")) %>%
...
data = data %>% # required
  ...
  mutate(
    ...
    Owner_country = "mex",
    System_name_english = "National Fishing Card for Sailfish", # required
    System_name_localized = "Carta Nacional Pesquera",
    System_code = "SFA NFC", # required
    System_code_localized = "SFA CNP",
    System_code_official = "0", # required
    System_multispecies = "0",
    System_species_description = paste("Istiophorus platypterus"),
    ...
    System_shape_file = "pez_vela.shp",
    ...
    Area_systematic_name_english = "Capture Area for Sailfish", # required
...
saveRDS(data, file = file.path(basedir, "processed", "country", "mexico", "mexico_28.Rds"))
