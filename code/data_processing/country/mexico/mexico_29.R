...
# Read data
data_orig <- st_read(file.path(basedir, "raw", "country", "mexico", "camaron_siete_barbas.shp")) %>%
...
data = data %>% # required
  ...
  mutate(
    ...
    Owner_country = "mex",
    System_name_english = "National Fishing Card for Pink Srimp", # required
    System_name_localized = "Carta Nacional Pesquera",
    System_code = "PSY NFC", # required
    ...
    System_shape_file = "camaron_siete_barbas.shp",
...
saveRDS(data, file = file.path(basedir, "processed", "country", "mexico", "mexico_29.Rds"))
