...
data_orig <- st_read(file.path(basedir, "raw", "MR_EEZ", "eez_v11.shp")) %>%
  filter(grepl("Bangladesh", GEONAME)) %>%
...
data <- data_orig %>%
  ...
  mutate(
    ...
    System_name_english = "Bangladesh", # required
    System_code = "bdg", # required
    ...
    System_lineage = "Filtered eez_v11.shp to so that GEONAME contained \"Bangladesh\"", # required
    System_type = "Bangladesh EEZ", # required
    System_category = "Management Area", # required
    Area_systematic_name_english = "Bangladesh EEZ", # required
...
saveRDS(data, file = file.path(basedir, "processed", "country", "bangladesh", "bangladesh_1.Rds"))
