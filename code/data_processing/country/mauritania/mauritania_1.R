...
data_orig <- st_read(file.path(basedir, "raw", "MR_EEZ", "eez_v11.shp")) %>%
  filter(grepl("Mauritania", GEONAME)) %>%
...
data <- data_orig %>%
  ...
  mutate(
    ...
    System_name_english = "Mauritania", # required
    System_code = "mrt", # required
    ...
    System_lineage = "Filtered eez_v11.shp to so that GEONAME contained \"Mauritania\"", # required
    System_type = "Mauritania EEZ", # required
    System_category = "Management Area", # required
    Area_systematic_name_english = "Mauritania EEZ", # required
...
saveRDS(data, file = file.path(basedir, "processed", "country", "mauritania", "mauritania_1.Rds"))
