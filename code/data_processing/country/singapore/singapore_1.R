...
data_orig <- st_read(file.path(basedir, "raw", "MR_EEZ", "eez_v11.shp")) %>%
  filter(grepl("Singapore", GEONAME)) %>%
...
data <- data_orig %>%
  ...
  mutate(
    ...
    System_name_english = "Singapore", # required
    System_code = "sgp", # required
    ...
    System_lineage = "Filtered eez_v11.shp to so that GEONAME contained \"Singapore\"", # required
    System_type = "Singapore EEZ", # required
    System_category = "Management Area", # required
    Area_systematic_name_english = "Singapore EEZ", # required
...
saveRDS(data, file = file.path(basedir, "processed", "country", "singapore", "singapore_1.Rds"))
