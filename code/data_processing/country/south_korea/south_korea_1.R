...
data_orig <- st_read(file.path(basedir, "raw", "MR_EEZ", "eez_v11.shp")) %>%
  filter(grepl("South Korea", GEONAME)) %>%
...
data <- data_orig %>%
  ...
  mutate(
    ...
    System_name_english = "Republic of Korea", # required
    System_name_localized = "대한민국",
    System_code = "kor", # required
    ...
    System_lineage = "Filtered eez_v11.shp to so that GEONAME contained \"South Korea\"", # required
    System_type = "Republic of Korea EEZ", # required
    System_category = "Management Area", # required
    Area_systematic_name_english = "Republic of Korea EEZ", # required
...
saveRDS(data, file = file.path(basedir, "processed", "country", "south_korea", "south_korea_1.Rds"))
